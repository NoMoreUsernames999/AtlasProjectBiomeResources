...
public class GenLayerRiverOverride extends GenLayer {

    private final Biome river;

    public GenLayerRiverOverride(long seed, GenLayer parent, Biome river) {
        super(seed);
        super.parent = parent;
        this.river = river;
    }
    ...
    public @Nonnull int[] getInts(int areaX, int areaY, int areaWidth, int areaHeight) {
        ...
        for (int i1 = 0; i1 < areaHeight; ++i1) {
            for (int j1 = 0; j1 < areaWidth; ++j1) {
                ...
                if (k2 == k1 && k2 == i2 && k2 == l1 && k2 == j2) aint1[j1 + i1 * areaWidth] = -1;
                else aint1[j1 + i1 * areaWidth] = Biome.getIdForBiome(this.river);
            }
        }
        return aint1;
    }
